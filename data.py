...
class Subject:
    ...
    def __init__(self, name, label_df):
        """
        variables:
            name(subject name)
        ...
        self.pulse_data.append(self._read_pulse(
        ...
        self.pulse_length = self.get_pulse_length()

    def _read_pulse(self, dir_name):
        """
        Reads the BPM csv files. Called in __init__.
        """
    ...
    def get_pad_pulse_data(self, pad_pulse_length):
        """
...
def readData(pulse_dir, label_filename):
    """
    ...
    for i in range(len(subject_names)):
        try:
            subject_list.append(
                Subject(subject_names[i], label_df=label_file.iloc[i * 20:(i + 1) * 20:, 1:]))
        except Exception as e:
            print(e)
...
class SubjectGroup:
    ...
    def __init__(self, pulse_dir, label_filename):
        self.subject_list = readData(pulse_dir, label_filename)
        ...
        for i in range(len(self)):
            if exclude_subject_name_list is not None:
                if self.subject_list[i].name in exclude_subject_name_list:
                    continue
            subject_data, subject_label = self.subjecååt_list[i].get_combined_data(
...
if __name__ == '__main__':
    ...
    parser.add_argument('-dir', '--pulse_data_dir',
                        default='data_pulse', help='directory to the pulse data')
    args = parser.parse_args()

    print(args)

    # subject_list = readData(args.pulse_data_dir, label_filename='label.xlsx')
    # print(subject_list[0])

    subject_group = SubjectGroup(
        pulse_dir=args.pulse_data_dir, label_filename='label.xlsx')
    ...
    print(data.shape)
    print(label.shape)
    np.save('data/data_all_600.npy', data)
    np.save('data/label_all_600.npy', label)
